...
const chartPositions = computed(() => {
    ...
    const othersPercent = small.reduce(
        (sum, p) => sum + p.calculatedPercent,
        0,
    );
    ...
    return [...large, others].sort(
        (a, b) => b.calculatedPercent - a.calculatedPercent,
    );
});
            ...
            labels: {
                ...
                generateLabels: (chart) => {
                    ...
                    return labels.map((label, index) => {
                        ...
                        const percentage =
                            position?.calculatedPercent.toFixed(1) ?? "0.0";
                        ...
                        return {
                            ...
                            strokeStyle: Array.isArray(borderColor)
                                ? borderColor[index]
        ...
        tooltip: {
            ...
            callbacks: {
                ...
                label: (context) => {
                    ...
                    const currency =
                        portfolio.value?.accountInfo.baseCurrency ?? "USD";
...
<template>
    <Card
        class="pie-card surface-0 border-1 surface-border border-round-xl shadow-2 overflow-hidden flex flex-column justify-content-center"
    >
        <template #header>
            <div
                class="flex justify-content-between align-items-center p-3 pb-0 mb-2 header-compact"
            >
                <h3
                    class="text-xl font-semibold text-900 m-0 flex align-items-center gap-2"
                >
                    <i class="pi pi-chart-pie text-primary"></i>
                    Asset Allocation
                </h3>
                <div
                    class="text-sm font-medium text-600 bg-gray-50 px-3 py-1 border-round"
                >
                    Total:
                    {{
                        formatCurrency(
                            portfolio?.totals.marketValue ?? 0,
                            portfolio?.accountInfo.baseCurrency,
                        )
                    }}
                </div>
            </div>
        ...
        <template #content>
            ...
            <div v-else class="text-center py-6 text-600">
                ...
                <p class="text-sm line-height-3">
                    Upload a portfolio CSV file to see your asset allocation.
                </p>
            </div>
        </template>
    </Card>
